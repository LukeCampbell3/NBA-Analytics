...
from .candidate_adapter import PairCandidate, build_candidates_for_day, build_pregame_action_rows

# Frozen once from DEVELOPMENT_STAMPS (see freeze_calibration.py) -- the
# calibration warm-up state this live adapter starts from. Not re-derived
# per run; a real forward-only calibration ledger (accumulating one
# aps_score per SETTLED day) is the natural next step once real
# prospective days exist (see MIGRATION notes) -- until then this frozen
# starting point is used unchanged, exactly like h_over_ranker's frozen
# H_BIAS.
...
def build_slate_payload(
    *,
    ...
    # Cheap short-circuit: with fewer than MIN_CALIBRATION_SLATES_FOR_STATE_SUPPORT
    # real prospective slates behind it, EVERY candidate's state_support
    # (and therefore in_support) is False regardless of legs -- skip the
    # expensive O(n^2) pair enumeration and abstain immediately, rather
    # than building tens of thousands of certificates only to discard them
    # all downstream.
    from .candidate_adapter import MIN_CALIBRATION_SLATES_FOR_STATE_SUPPORT

    if FROZEN_CALIBRATION_SLATES < MIN_CALIBRATION_SLATES_FOR_STATE_SUPPORT:
    ...
    #
    # support["in_support"] filter: this is a DESCRIPTIVE diagnostic from
    # the adapter (leg-level history/RMSE support AND state-level
    # calibration support), not itself a certification -- but a candidate
    # the adapter itself doesn't consider well-supported should never
    # reach the policy at all. With FROZEN_CALIBRATION_SLATES=0 (no real
    # prospective calibration has accumulated yet -- see the module
    # docstring), state_support is always False right now, so this
    # correctly abstains every day until real evidence accumulates, rather
    # than certifying against a calibration threshold with zero real
    # history behind it.
    cross_game_candidates = [c for c in candidates if c.leg_1.game_id != c.leg_2.game_id and c.support["in_support"]]

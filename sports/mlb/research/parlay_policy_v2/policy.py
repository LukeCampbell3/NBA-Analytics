"""Leakage-safe two-leg parlay policy utilities (MLB port).

Byte-for-byte the same mechanism as
sports/nba/predictions/Player-Predictor/research/parlay_policy_v2/policy.py
(ported, not reimplemented) after that gate was backtested for real against
this sport's own settled candidate pairs -- see real_data_backtest.py and
REPORT.md for the numbers and every caveat on what was and wasn't exercised.
...
wagers, reads a production board, or is imported by CONTROL
(sports/parlay_analysis.py) or by MLB's existing, much more heavily
instrumented parlay_certification_v2 / joint_position_builder_v2 programs.
See REPORT.md for how this relates to that existing program and why this
module is not production-authorized either.
"""

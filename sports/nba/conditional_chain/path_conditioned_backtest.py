...
MECHANISM_STATUS = "REAL_NBA_PATH_MECHANISM_IMPLEMENTED_INCREMENTAL_VALUE_UNPROVEN"
PRIMARY_LEG_COUNT = 2
...
def _prepare_reservoir(frame: pd.DataFrame) -> pd.DataFrame:
    required = {"event_date", "player", "market", "side", "robust_score", "leg_result"}
...
def _ablation_report(evaluated: pd.DataFrame, path_certificate: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        status = "REAL_NBA_PATH_INCREMENTAL_VALUE_SUPPORTED"
    elif path_valid and enough:
        status = "REAL_NBA_PATH_INCREMENTAL_VALUE_NOT_SUPPORTED"
    else:
        status = MECHANISM_STATUS
    return {
        "status": status,

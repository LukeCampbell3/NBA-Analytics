...
Why MLB and not NBA: this repository has no settled NBA two-leg parlay
dataset carrying real leg-level probability/result/price (see REPORT.md).
MLB does -- `sports/mlb/data/predictions/backtests/mlb_walk_forward_backtest_rows.csv`,
...
real American side price). This script uses that real data to answer two
separate questions with real numbers:

  1. What does sports/parlay_analysis.py (the CONTROL/production module,
     shared by every sport including NBA) actually score on these real,
     settled legs? (unmodified import, no changes to that module)
...
IMPORTANT CAVEAT: this is a demonstration of the *mechanism* against real
numbers, not an NBA performance claim, and not a full exercise of
parlay_policy_v2 -- this dataset does not log per-candidate joint_sigma,
shared_failure_risk, compatible_state_score, shift_risk, or lineup/role/
injury/support state, so those gates are left at pass-through defaults
below and are NOT exercised. Only the probability-floor and
actual-quote-EV-from-real-decimal-price mechanism is exercised for real.
Cross-game legs use the product of each leg's own real decimal price as the
actual quote -- CONTROL's own documented convention for a straight,
non-same-game parlay (see sports/mlb/research/joint_position_builder_v2/REPORT.md),
not a synthetic substitute for a same-game SGP quote.

Run: python3 sports/nba/predictions/Player-Predictor/research/parlay_policy_v2/real_data_backtest_mlb.py
"""
...
from sports.parlay_analysis import evaluate_historical_parlays, score_candidate_parlays  # noqa: E402

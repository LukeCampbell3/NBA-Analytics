#!/usr/bin/env python3
"""Real, deduplicated local headshot cache for NBA's real bettable players.

Mirrors sports/mlb/scripts/update_mlb_player_headshot_cache.py's design
(see its module docstring for the full rationale) for NBA's board, which
has no parlay leg product -- only export_daily_predictions_web.py's real
main-board plays, each already carrying a real
cdn.nba.com/headshots/nba/latest/1040x760/{player_id}.png URL built from
the real numeric NBA player id.
"""
...
DEFAULT_CACHE_DIR = REPO_ROOT / "sports" / "nba" / "web" / "data" / "headshots"
DEFAULT_MANIFEST_PATH = DEFAULT_CACHE_DIR / "manifest.json"
LOCAL_URL_PREFIX = "data/headshots"

PLAYER_ID_PATTERN = re.compile(r"/headshots/nba/latest/[^/]+/(\d+)\.png")
...
def find_headshot_dicts(payload: dict[str, Any]) -> list[dict[str, Any]]:
    return [play for play in (payload.get("plays") or []) if isinstance(play, dict) and play.get("player_headshot_url")]

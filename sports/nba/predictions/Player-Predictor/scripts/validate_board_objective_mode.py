...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["edge", "abs_edge", "ev_adjusted", "robust_reranker", "precision_pool", "thompson_ev", "board_objective"],
...
def _policy_kwargs(payload: dict, mode: str) -> dict:
    return {
        ...
        "target_direction_allocation_enabled": payload.get("target_direction_allocation_enabled", False),
        "target_direction_min_rows": payload.get("target_direction_min_rows", 80),
        "target_direction_strong_rate": payload.get("target_direction_strong_rate", 0.64),
        "target_direction_weak_rate": payload.get("target_direction_weak_rate", 0.56),
        "target_direction_very_weak_rate": payload.get("target_direction_very_weak_rate", 0.52),
        "target_direction_max_weak_cap": payload.get("target_direction_max_weak_cap", 1),
        "target_direction_max_very_weak_cap": payload.get("target_direction_max_very_weak_cap", 0),
        "target_direction_weak_penalty_weight": payload.get("target_direction_weak_penalty_weight", 0.035),
        "target_direction_recent_weakness_penalty_weight": payload.get("target_direction_recent_weakness_penalty_weight", 0.030),
        "target_direction_reserve_max_slots": payload.get("target_direction_reserve_max_slots", 2),
        "target_direction_reserve_min_prob": payload.get("target_direction_reserve_min_prob", 0.54),
        ...
        "precision_pool_target_accuracy": payload.get("precision_pool_target_accuracy", 0.83),
        "precision_pool_recent_days": payload.get("precision_pool_recent_days", 14),
        "precision_pool_prior_strength": payload.get("precision_pool_prior_strength", 18.0),
        "precision_pool_miss_penalty": payload.get("precision_pool_miss_penalty", 2.4),
        "precision_pool_volume_reward": payload.get("precision_pool_volume_reward", 0.015),
    }
                ...
                rows.append(
                    {
                        ...
                        "precision_pool_enabled": bool(pd.to_numeric(pd.Series([row.get("precision_pool_enabled")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_prob": float(pd.to_numeric(pd.Series([row.get("precision_pool_prob")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_lcb": float(pd.to_numeric(pd.Series([row.get("precision_pool_lcb")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_score": float(pd.to_numeric(pd.Series([row.get("precision_pool_score")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_support_rows": float(pd.to_numeric(pd.Series([row.get("precision_pool_support_rows")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_short_prob": float(pd.to_numeric(pd.Series([row.get("precision_pool_short_prob")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_long_prob": float(pd.to_numeric(pd.Series([row.get("precision_pool_long_prob")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_consistency_score": float(pd.to_numeric(pd.Series([row.get("precision_pool_consistency_score")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_regime_delta": float(pd.to_numeric(pd.Series([row.get("precision_pool_regime_delta")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_regime_trust": float(pd.to_numeric(pd.Series([row.get("precision_pool_regime_trust")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_feedback_prob": float(pd.to_numeric(pd.Series([row.get("precision_pool_feedback_prob")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_feedback_lcb": float(pd.to_numeric(pd.Series([row.get("precision_pool_feedback_lcb")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_feedback_rows": float(pd.to_numeric(pd.Series([row.get("precision_pool_feedback_rows")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_feedback_adjustment": float(pd.to_numeric(pd.Series([row.get("precision_pool_feedback_adjustment")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_market_strength": float(pd.to_numeric(pd.Series([row.get("precision_pool_market_strength")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_dynamic_size": float(pd.to_numeric(pd.Series([row.get("precision_pool_dynamic_size")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_target_attainable": bool(pd.to_numeric(pd.Series([row.get("precision_pool_target_attainable")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_selected_avg_prob": float(pd.to_numeric(pd.Series([row.get("precision_pool_selected_avg_prob")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_selected_avg_lcb": float(pd.to_numeric(pd.Series([row.get("precision_pool_selected_avg_lcb")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_selected_avg_consistency": float(pd.to_numeric(pd.Series([row.get("precision_pool_selected_avg_consistency")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_selected_avg_strength": float(pd.to_numeric(pd.Series([row.get("precision_pool_selected_avg_strength")]), errors="coerce").fillna(np.nan).iloc[0]),
                        "precision_pool_history_rows": float(pd.to_numeric(pd.Series([row.get("precision_pool_history_rows")]), errors="coerce").fillna(0).iloc[0]),
                        "precision_pool_recent_rows": float(pd.to_numeric(pd.Series([row.get("precision_pool_recent_rows")]), errors="coerce").fillna(0).iloc[0]),
        ...
        summary_rows.append(
            {
                ...
                "precision_pool_enabled_rate": float(pd.to_numeric(part.get("precision_pool_enabled"), errors="coerce").fillna(0).mean()) if "precision_pool_enabled" in part.columns else np.nan,
                "avg_precision_pool_prob": float(pd.to_numeric(part.get("precision_pool_prob"), errors="coerce").mean()) if "precision_pool_prob" in part.columns else np.nan,
                "avg_precision_pool_lcb": float(pd.to_numeric(part.get("precision_pool_lcb"), errors="coerce").mean()) if "precision_pool_lcb" in part.columns else np.nan,
                "avg_precision_pool_score": float(pd.to_numeric(part.get("precision_pool_score"), errors="coerce").mean()) if "precision_pool_score" in part.columns else np.nan,
                "avg_precision_pool_consistency": float(pd.to_numeric(part.get("precision_pool_consistency_score"), errors="coerce").mean()) if "precision_pool_consistency_score" in part.columns else np.nan,
                "avg_precision_pool_regime_trust": float(pd.to_numeric(part.get("precision_pool_regime_trust"), errors="coerce").mean()) if "precision_pool_regime_trust" in part.columns else np.nan,
                "avg_precision_pool_feedback_prob": float(pd.to_numeric(part.get("precision_pool_feedback_prob"), errors="coerce").mean()) if "precision_pool_feedback_prob" in part.columns else np.nan,
                "avg_precision_pool_feedback_lcb": float(pd.to_numeric(part.get("precision_pool_feedback_lcb"), errors="coerce").mean()) if "precision_pool_feedback_lcb" in part.columns else np.nan,
                "avg_precision_pool_feedback_adjustment": float(pd.to_numeric(part.get("precision_pool_feedback_adjustment"), errors="coerce").mean()) if "precision_pool_feedback_adjustment" in part.columns else np.nan,
                "avg_precision_pool_market_strength": float(pd.to_numeric(part.get("precision_pool_market_strength"), errors="coerce").mean()) if "precision_pool_market_strength" in part.columns else np.nan,
                "precision_pool_target_attainable_rate": float(pd.to_numeric(part.get("precision_pool_target_attainable"), errors="coerce").fillna(0).mean()) if "precision_pool_target_attainable" in part.columns else np.nan,
            }
        )

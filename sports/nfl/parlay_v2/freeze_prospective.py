...
"""Freeze-readiness / prospective-freeze command. Ported from
sports/mlb/parlay_v2/freeze_prospective.py.

    python -m sports.nfl.parlay_v2.freeze_prospective --policy NFL_PARLAY_POLICY_V2_PROSPECTIVE_001 [--confirm]
...
from sports.nfl.research.parlay_certification_v2 import manifest, prospective_boundary
from sports.nfl.research.parlay_certification_v2.anytime_monitor import REFERENCE_MONITOR_VERSION
from sports.nfl.research.parlay_certification_v2.evidence_store import EVIDENCE_STORE_VERSION
from sports.nfl.research.parlay_certification_v2.settlement import SETTLEMENT_VERSION
from sports.nfl.research.parlay_certification_v2.state_machine import STATE_MACHINE_VERSION
from sports.nfl.research.parlay_certification_v2.world_certificate import WORLD_CERTIFICATE_VERSION
...
BOUNDARY_ROOT = REPO_ROOT / "sports" / "nfl" / "research" / "parlay_certification_v2" / "reports" / "prospective_boundary"
READINESS_ROOT = REPO_ROOT / "sports" / "nfl" / "parlay_v2" / "reports"
...
RELEVANT_SOURCE_PATHS = (
    "sports/nfl/parlay_v2",
    "sports/nfl/research/parlay_certification_v2",
    "sports/nfl/conditional_chain",
)
...
def build_readiness_artifact(policy_id: str) -> dict:
    ...
    # readiness does NOT wait on them. The three REQUIRED dimensions --
    # market_support/line_support/state_support -- are real, implemented,
    # and frozen with real thresholds (N_MARKET/N_LINE/N_STATE) already;
    # whether any given candidate currently PASSES them is a per-week
    # ledger-content question, not a freeze-readiness (code/config)
    # question, so it is correctly not checked here either.
    ...
    readiness = {
        ...
        "candidate_enumeration": "all cross-event 2-leg pairs from the weekly action-eligible universe (candidate_adapter.build_candidates_for_week)",
        "world_gate_mode": manifest.WORLD_GATE_MODE,
        "tie_breaker": (
            ...
            else "ascending world_risk_rho (inherited from MLB's DEVELOPMENT-validated ranking diagnostic; see sports/mlb/research/parlay_certification_v2/world_gate_research.py), then lexicographic wager_id"
        ),
        "max_actions_per_eligible_slate": manifest.MAX_ACTIONS_PER_ELIGIBLE_SLATE,
        "leg_count": 2,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Freeze-readiness check / one-way prospective boundary activation for an NFL PARLAY_POLICY_V2 stream.")
    parser.add_argument("--policy", required=True, help="Policy id, e.g. NFL_PARLAY_POLICY_V2_PROSPECTIVE_001")
